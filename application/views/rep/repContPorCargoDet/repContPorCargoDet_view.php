...
<div id="content">
<h1>Reporte de Conteo por Cargo Detallado</h1>
...
<script>
	...
	function consultarResults(){
		dataString = "<?php echo site_url('reporte/repContPorCargoDetHtml/'); ?>";
	...
	function genRepContPorCargo(){
		...
		html_content += '<th>PLANILLA</th>';
		html_content += '<th>VOTOS</th>';
		html_content += '<th>PORCENTAJE</th>';
		html_content += '</tr>';
		html_content += '</thead>';
		html_content += '<tbody>';
		for( var i=0; i<tabla.length; i++ ){
			...
			html_content += '</td>';
			html_content += '<td class="textCenter">';
			html_content +=	$( "#planilla" + tabla[i].puesto_id ).val();
			html_content += '</td>';
			html_content += '<td class="textCenter">';
			html_content +=	$( "#votos" + tabla[i].puesto_id ).val();
			html_content += '</td>';
			html_content += '<td class="textCenter">';
			html_content +=	$( "#pcntj" + tabla[i].puesto_id ).val() + "%";
			html_content += '</td>';
			html_content += '</tr>';
		}
		html_content += '</tbody>';
		...
		printReport(urlSitio + 'reporte/repContPorCargoDetPdf/' );
	}	
</script>
